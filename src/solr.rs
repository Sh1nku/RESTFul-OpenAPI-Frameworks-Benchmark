...
pub fn create_solr_client(host: &str) -> AsyncSolrCloudClient {
    AsyncSolrCloudClient::new(SolrServerContextBuilder::new(SolrSingleServerHost::new(
        host,
    )))
}
...
pub async fn upload_data_to_solr(client: &AsyncSolrCloudClient) -> Result<(), Box<dyn Error>> {
    ...
    client
        .upload_config(CONFIG_NAME, Path::new("benchmark_server/solrconfig"))
        .await?;
    client
        .create_collection(COLLECTION_NAME, CONFIG_NAME, 1, 1)
        .await?;
...
async fn upload_test_data(
    ...
    let data: Vec<Entity> = (0..100)
        .map(|i| Entity {
            id: i.to_string(),
            document_type: 1,
            int_array: (0..10).map(|_| rng.gen_range(0..10)).collect(),
            string_array: (0..10)
                .map(|_| hex::encode(rng.gen::<[u8; 16]>()))
                .collect(),
            child_objects: (0..10)
                .map(|j| SubEntity {
                    id: format!("{i}_{j}"),
        ...
        .collect();
    client
        .index(&UpdateQuery::new(), collection_name, &data)
        .await?;
    Ok(())
}
